...
export const documentService = {
  ...
  async processDocument(documentId) {
    ...
    const { pages, metadata } = await parseUpload(
      { buffer: await storage.readFile(document.storageKey) },
      document.fileType,
    );
    ...
    const embeddedChunks = await embeddings.embedBatch(chunkTexts).then((vectors) =>
  ...
  async getForUser(documentId, userId, { includeContent = false } = {}) {
    ...
    if (!isOwner(document, userId)) throw new ApiError(403, 'Not authorized to view this document');
  ...
  async delete(documentId, userId) {
    const document = await documentRepository.findById(documentId);
    if (!document) throw new ApiError(404, 'Document not found');
    if (!isOwner(document, userId)) throw new ApiError(403, 'Not authorized');
...
function buildFilters(query) {
  ...
  if (query.favorite === 'true') filters.isFavorite = true;
  if (query.period === 'recent') filters.createdAt = { $gte: new Date(Date.now() - 7 * 86400000) };
  return filters;
}
