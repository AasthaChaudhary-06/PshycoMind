...
export const progressService = {
    ...
    async trackReading(userId, { documentId, page, totalPages }) {
        const progress = await progressRepository.getOrCreate(userId);
        const entryIndex = progress.documents.findIndex((d) => d.document.toString() === documentId);
        ...
        progress.totalPagesRead = progress.documents.reduce((sum, d) => sum + (d.pagesRead || 0), 0);
