...
export const documentRepository = {
  ...
  async findById(id, { includeContent = false } = {}) {
    ...
    if (includeContent) query.select('+pages +chunks');
    else query.select('-pages -chunks');
    return query.exec();
  },
  ...
  async findPaginatedForUser(userId, { page, limit, sort, filters = {} }: any) {
    const query: any = { ...BASE_FILTERS, owner: userId, ...filters };
