...
export const tutorService = {
  ...
  async handle(userId, { content, documentId }) {
    ...
    if (!text) return { intent: 'ask', response: 'Please type a message.' };
...
function classifyIntent(text) {
  ...
  if (/\b(summariz|summary|tl;?dr)\b/.test(t)) return 'summarize';
  if (/\b(quiz|mcq|question|test me)\b/.test(t)) return 'quiz';
  if (/\b(flashcard|study card|anki)\b/.test(t)) return 'flashcards';
  if (/\b(exam|mock test|paper)\b/.test(t)) return 'exam';
  if (/\b(plan|schedule|syllabus|routine)\b/.test(t)) return 'plan';
  if (/\b(explain|what is|what are|define|how does|why)\b/.test(t)) return 'explain';
  return 'ask';
}
