...
const XP_AWARDS: Record<string, { xp: number; counter?: keyof any }> = {
...
export const gamificationService = {
  async awardXp(userId: string, reason: string, extra: { bestQuizScore?: number } = {}) {
  ...
  async getProfile(userId: string) {
    ...
    return {
      ...
      badges: (user.badges || []).map((id: string) => BADGES.find((b) => b.id === id) || { id, name: id, description: '', icon: '🏅' }),
      stats: user.stats || {},
    };
  },

  async getLeaderboard({ limit = 50 }: { limit?: number } = {}) {
    return User.find({})
