...
export const graphService = {
  async generate(userId: string, documentId: string) {
        ...
        [
          {
            role: 'system',
            content:
              'You are a medical knowledge graph builder. Extract the key concepts and their relationships. Return ONLY a JSON object with "title", "nodes" (id, label, explanation, page, importance 1-5) and "edges" (source, target, relation). Keep ids short and unique.',
          },
  ...
  async getForUser(userId: string, documentId: string) {
    ...
    if (!graph) throw new ApiError(404, 'Knowledge graph not found. Generate it first.');
    return graph;
  },
};

function buildContext(document: any) {
  const chunks = Array.isArray(document.chunks) ? document.chunks : [];
  let out = '';
  for (const chunk of chunks) {
    const text = (chunk.text || '').trim();
    if (!text) continue;
    if (out.length + text.length > MAX_CONTEXT_CHARS) break;
    out += `[Page ${chunk.page}]\n${text}\n\n`;
  }
  return out || (document.pages || []).map((p: any) => `[Page ${p.number}]\n${p.text}`).join('\n\n') || 'No content.';
}

function parseGraph(content: any) {
  try {
